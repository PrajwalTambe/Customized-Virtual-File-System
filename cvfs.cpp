...
void DisplayHelp()
{
	...
	printf("exit: To terminates file system \n");
	printf("stat: To Display information of file using name\n");
	printf("fstat: To Display information of file using ifle descriptor\n");
...
{
	if(((UFDTArr[fd].ptrfiletable->mode)!=WRITE)&&((UFDTArr[fd].ptrfiletable->mode)
		!=READ+WRITE))return -1;
...
int LseekFile(int fd, int size,int from)
{
	...
	{ 	
		if(from == CURRENT)
		{
			if(((UFDTArr[fd].ptrfiletable->readoffset)+size)>UFDTArr[fd].ptrfiletable
...
while(temp != NULL)
{
	...
	{
		printf("%s\t\t%d\t\t%d\t\t%d\n",temp->
		FileName,temp->InodeNumber,temp->FileActualSize,temp->LinkCount);
	}
	temp=temp->next;
}	
	...
	while(1)
	{
		...
		else if(count == 3)
		{
			...
			else if(strcmp(command[0],"read")==0)
			{
				...
				if(ret>=-1)
					printf("Filre not existing\n");
				if(ret == -2)
					printf("ERROR: Permission denied\n");
				if(ret == -3)
					printf("ERROR: Reach at the end of file\n");
				if(ret == -4)
					printf("ERROR: It is not regular file\n");
				  if(ret == 0)
